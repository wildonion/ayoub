...
impl ThreadPool{
    ...
    pub fn new(size: usize) -> ThreadPool {
        ...
        let receiver = Arc::new(Mutex::new(receiver)); //-- we want to receive message inside other threads thus we have to share the receiver between threads using Arc<Mutex<Receiver>> - reading and writing from an IO must be mutable thus the receiver must be inside a Mutex cause data inside Arc can't be borrows as mutable since the receiver read operation is a mutable process

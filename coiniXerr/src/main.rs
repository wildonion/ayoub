...
NOTE - we can save each tokio::spawn() inside a vector of type JoinHandle (like OS threads) to await on them one by one later 
...
NOTE - tokio::spawn() is an asynchronous multithreaded (green threads) and event loop based task spawner and scheduler which takes 
...
NOTE - we can't borrow data inside Arc as mutable if we have a an object in which one of its method has &mut self as its first argument 
       and needs to mutate a field like run_time_info add() method in which the info_dict field will be updated. 

NOTE - to solve above issue we have to put that object inside a Mutex (&mut) to share its ownership (Arc) and protect it between multiple 
...
NOTE - & is used to take a reference or borrow the ownership; for Copy trait bounded type this means coping the type by borrowing 
       its ownership and &mut is a mutable pointer to that for changing it.

NOTE - Arc (we use Arc if the type wasn't bounded to Clone trait) is used to share the ownership of data and move it between threads 
...
NOTE - Box is a smart pointer and is used along with the dyn keyword instread of &dyn to take 
       a reference to object safe traits which are not bounded to Sized and Copy traits.

NOTE - when an actor is created, it gets its own mailbox for receiving messages and other interested actors are notified about the new actor joining the system

NOTE - ActorRef is a lightweight type that is inexpensive to clone and can be used to interact with its underlying Actor, 
       such as sending messages to it also is a reference to the actor.

NOTE - ActorRef always refers to a specific instance of an actor, when two instances of the same Actor are started, 
       they're still considered separate actors, each with different ActorRefs.

NOTE - ActorRef are inexpensive and can be cloned alos they can be sent as a message to another actor.

NOTE - remote_handle() method turns this future into a future that yields () on completion and sends its output to another future on a separate task
...
async fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync + 'static>>{ //// bounding the type that is caused to error to Error, Send and Sync traits to be shareable between threads and have static lifetime across threads and awaits
    ...
    // TODO - update the slot field with a new slot based on auction biddings and some random selection algo
    // TODO - send message of type Communicate from parachain_0 (first shard) to parachain_1 (second shard)
    // ...
    ...
    let parachain_1 = coiniXerr_sys.actor_of_props::<actors::parathread::Parachain>("parachain_0", parachain_1_props.clone()).unwrap(); //-- initializing the second parachain actor with its props; ActorRef is of type ParachainMsg means that we can communicate with another actor or the actor itself by sending Validator iteself as a message - props are Clone and Send and we can share them between threads
    // ----------------------------------------------------------------------
    ...
    //// we have to ask the actor that hey we want some info about the parachain by sending the related message like getting the current block cause the parachain is guarded by the ActorRef
    ...
    while let Ok((stream, addr)) = listener.accept().await{ //-- await suspends the accept() function execution to solve the future but allows other code blocks to run  
        ...
        // ----------------------------------------------------------------------
        info!("-> {} - starting validator actor for this peer", chrono::Local::now().naive_local());
        let validator_props = Props::new_args::<Validator, _>((Uuid::new_v4(), addr.clone(), None, actors::peer::Mode::Mine, None));
        let validator_actor = coiniXerr_sys.actor_of_props::<Validator>("validator", validator_props.clone()).unwrap(); //-- initializing the validator actor with its props; ActorRef is of type ValidatorMsg means that we can communicate with another actor or the actor itself by sending Validator iteself as a message - props are Clone and Send and we can share them between threads
        ...
        pool.execute(move || { //-- parallel transactions handler - executing pool of threads for scheduling asynchronous tasks spawned with tokio::spawn() using a messaging channel protocol called mpsc job queue channel in which its sender will send the job or task or message coming from the process constantly to the channel and the receiver inside an available thread (a none blocked thread) will wait until a job becomes available to down side of the channel finally the current thread must be blocked for the mutex (contains a message like a job) lock - mpsc definition : every job or task has its own sender but only one receiver can be waited at a time inside a thread for mutex lock 
            tokio::spawn(async move { //-- spawning an async task (of socket process) inside a thread pool which will use a thread to start a validator actor in the background - a thread will be choosed to receive the task or job using the down side of the mpsc channel (receiver) to acquire the mutex for the lock operation
                ...
                stream_sender.send((stream, meta_data_uuid, cloned_arc_mutex_runtime_info_object, cloned_arc_mutex_validator_actor)).await.unwrap(); //-- sending the stream, the cloned runtime info and metadata uuid through the mpsc channel 
            ...
            while match stream.read(&mut transaction_buffer_bytes).await{ //-- streaming over the incoming bytes from the socket - reading is the input and writing is the output
                ...
                Ok(size) => {
                    ...
                    if must_be_signed && deserialized_transaction_borsh.issued < now{ //// the downside of the mempool channel must be available and also the issued time of the transaction must be smaller than the current server time
                        ...
                        mempool_sender.send((cloned_arc_mutex_transaction, cloned_arc_mutex_validator_actor.clone())).await.unwrap(); //-- sending signed transaction plus the validator actor info through the mpsc job queue channel asynchronously for mining process - we must clone the cloned_arc_mutex_validator_actor in each iteration to prevent ownership moving
                        true
                    } else{
                        // ----------------------------------------------------------------------

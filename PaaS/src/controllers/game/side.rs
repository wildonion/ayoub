...
pub async fn add(db: Option<&Client>, api: ctx::app::Api) -> GenericResult<hyper::Response<Body>, hyper::Error>{
    ...
    api.post("/game/side/add", |req, res| async move{ // NOTE - api will be moved here since neither trait Copy nor Clone is not implemented for that and we can call it only once 
...
pub async fn all(db: Option<&Client>, api: ctx::app::Api) -> GenericResult<hyper::Response<Body>, hyper::Error>{
    ...
    api.post("/game/side/get/availables", |req, res| async move{ // NOTE - api will be moved here since neither trait Copy nor Clone is not implemented for that and we can call it only once     


        ////////////////////////////////// DB Ops
                        
        let filter = doc! { "is_disabled": false }; //-- filtering all none disabled sides
        let sides = db.unwrap().database("ayoub").collection::<schemas::game::SideInfo>("sides"); //-- selecting sides collection to fetch and deserialize all sides infos or documents from BSON into the SideInfo struct
...
pub async fn disable(db: Option<&Client>, api: ctx::app::Api) -> GenericResult<hyper::Response<Body>, hyper::Error>{
    ...
    api.post("/game/side/diable", |req, res| async move{ // NOTE - api will be moved here since neither trait Copy nor Clone is not implemented for that and we can call it only once 
                            ...
                            Ok(value) => { //-- making a serde value from the buffer which is a future IO stream coming from the client
                                ...
                                match serde_json::from_str::<schemas::game::DisableSideRequest>(&json){ //-- the generic type of from_str() method is DisableSideRequest struct - mapping (deserializing) the json string into the DisableSideRequest struct
                                    Ok(dis_info) => {
                                        ...
                                        let sides = db.unwrap().database("ayoub").collection::<schemas::game::SideInfo>("sides"); //-- selecting sides collection to fetch all side infos into the SideInfo struct

...
pub async fn get_single(db: Option<&Client>, api: ctx::app::Api) -> GenericResult<hyper::Response<Body>, hyper::Error>{ //-- this api will return the current status and infos of a player during the game and can be called by the God and the player
...
pub async fn get_player_role_ability(db: Option<&Client>, api: ctx::app::Api) -> GenericResult<hyper::Response<Body>, hyper::Error>{ //-- this api will return the current role ability of a specific player during the game and can be called by the God and the player

    info!("calling {} - {}", api.name, chrono::Local::now().naive_local()); //-- info!() macro will borrow the api and add & behind the scene

    api.post("/game/player/get/role-ability", |req, res| async move{ // NOTE - api will be moved here since neither trait Copy nor Clone is not implemented for that and we can call it only once 
...
pub async fn get_player_chain_infos(db: Option<&Client>, api: ctx::app::Api) -> GenericResult<hyper::Response<Body>, hyper::Error>{ //-- this api will return the current chain infos of a specific player during the game and can be called by the God and the player

    info!("calling {} - {}", api.name, chrono::Local::now().naive_local()); //-- info!() macro will borrow the api and add & behind the scene

    api.post("/game/player/get/chain-infos", |req, res| async move{ // NOTE - api will be moved here since neither trait Copy nor Clone is not implemented for that and we can call it only once 
                            ...
                            Ok(value) => { //-- making a serde value from the buffer which is a future IO stream coming from the client
                                ...
                                match serde_json::from_str::<schemas::game::GetPlayerInfoRequest>(&json){ //-- the generic type of from_str() method is GetPlayerInfoRequest struct - mapping (deserializing) the json string into the GetPlayerInfoRequest struct
                                    Ok(player_info) => { //-- we got the username and password inside the login route
                                        ...
                                        let player_chain_info = db.unwrap().database("ayoub").collection::<schemas::game::PlayerChainToInfo>("player_chain_info"); //-- connecting to player_chain_info collection to get a document - we want to deserialize player chain info into the PlayerChainToInfo struct                        

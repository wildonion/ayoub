...
pub async fn main(db: Option<&MC>, api: ctx::app::Api) -> GenericResult<hyper::Response<Body>, hyper::Error>{
    ...
    api.post("/auth/otp-req", |req, res| async move{ // NOTE - api will be moved here since neither trait Copy nor Clone is not implemented for that and we can call it only once 

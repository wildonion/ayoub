                    ...
                    Ok(otp_req) => { //-- we got the phone number of the user
                        ...
                        if sms_response_streamer.status() == 200{ /////// the status of the OTP career is 200 means the code has been sent successfully to the receiver
                            ...
                            while let Some(next) = sms_response_streamer.body_mut().data().await{ //-- bodies in hyper are always streamed asynchronously and we have to await for each chunk as it comes in using a while let Some() syntax
                                let chunk = next?;
                                write!(stream, "{:#?}", chunk).unwrap(); //-- collecting each incoming chunks to write them into the defined stream buffer to deserialize from utf8 bytes into the either SMSResponse or OTPCareerResponse struct to send back as json to user
                            }
                            ...
                            // --------------------------------------------------------------------
                            //    DESERIALIZING FROM ut8 BYTES INTO THE OTPCareerResponse STRUCT
                            // --------------------------------------------------------------------
                            match serde_json::from_slice::<schemas::auth::OTPCareerResponse>(stream.buffer()){ //-- we can also use from_reader() method which is slower than from_slice() method and deserialize the bytes of json text directly into the OTPCareerResponse struct - the generic type of from_slice() method is OTPCareerResponse struct - mapping (deserializing) the bytes of json text into the OTPCareerResponse struct
                                Ok(sms_response) => {
                                    


                                    let message_response = sms_response.extra.get("message").unwrap().to_string(); //-- converting the message value into string as a response message to send through the response object - getting the message value from the extra field of OTPCareerResponse struct which is flattened to capture all the incoming fields from the OTP career into a map with arbitrary string keys and values of type serde_json::Value since we don't know the type of each value or how many keys are in the OTP career response 
                                    let status_response = sms_response.extra.get("status").unwrap(); //-- getting the status value from from the extra field of OTPCareerResponse struct which is flattened to capture all the incoming fields from the OTP career into a map with arbitrary string keys and values of type serde_json::Value since we don't know the type of each value or how many keys are in the OTP career response

...
pub async fn main(db: Option<&Client>, api: ctx::app::Api) -> GenericResult<hyper::Response<Body>, hyper::Error>{
    ...
    api.post("/auth/login", |req, res| async move{ // NOTE - api will be moved here since neither trait Copy nor Clone is not implemented for that and we can call it only once 

        match middlewares::cors::is_preflight_request(req).await{
            Ok((_true, req)) => {
                middlewares::cors::send_preflight_response(req).await //-- sending preflight resposne to the browser to get rid of fucking CORS :))
            },
            Err((_false, req)) => {

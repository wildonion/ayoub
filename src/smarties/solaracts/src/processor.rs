...
/* 
        //////////////////////////////// FACTS
        //////-- a contract is a way to make trust between two accounts and only contains instructions like transfering lamports from some accounts to another account on a specific event or condition 
        //////-- the bpf bytecode of the contract will be sent as a transaction to the solana blokchcain through the JSONRPC using deploy command and from there will be run till the lifetime of the network inside all nodes thus we can call its methods on some conditions using its address or the deployed program id
...
pub fn escrow( //-- this program keeps track of the number of times the escrow() program has been called for a given account - every program belongs to a instruction and every instruction belongs to an entrypoint loader and all accounts are owned by a specific program
    ...
    accounts: &[AccountInfo], //-- accounts to say hello to which interact with this program store data related to program interaction like saving infos about uploaded metadata files for this account - accounts contain data and owner to save data inside an account for a specific owner and are held in validator memory and pay rent to stay there
    ...
    if writer.owner != program_id{ //-- writer.owner is the program id that ownes this account and is not controlled by a private key like other accounts cause accounts can only be owned by programs - the owner of this account which is called program derived account is the program id thus this account will be marked as executable accoutn
    ...
    if !payer.is_signer{ //-- payer or the funder account must be a signer to sign the transaction
    ...
    msg!("Signed {} time(s)! with the name {}", contract_account.sign, contract_account_string_val.sign); //-- msg!() has lower runtime cost than the println!() and info!()


    

    
    // TODO - this contract (a family tree contract) is our campaign in which will catch a commission from incoming lamports and transfer the rest to the family tree owner account
    // TODO - funder will send a transaction also contains some instruction data to transfer lamports from his/her address to our campaign address (escrow)

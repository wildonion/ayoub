...
impl Contract{
    ...
    pub fn nft_mint(&mut self, token_id: TokenId, metadata: TokenMetadata, receiver_id: AccountId){ //-- we've defined the self to be mutable and borrowed cause we want to mutate the state of token_metadata_by_id and tokens_by_id fields but don't want to lose the lifetime of the created instance of the contract after calling this method 
        ...
        let initial_storage_usage = env::storage_usage(); //-- measuring the initial storage being uses on the contract as u64 bits or 8 bytes 
        let token = Token{
            owner_id: receiver_id, //-- the receiver_id is the one who is minting this token and is the owner of the current token
        };
    
        assert!(self.tokens_by_id.insert(&token_id, &token).is_none(), "Token already minted!"); //-- inserting the token ID and the token struct into the tokens_by_id field to make sure that we didn't mint this token before
        self.token_metadata_by_id.insert(&token_id, &metadata); //-- inserting the token ID and its metadata into the token_metadata_by_id field
        self.internal_add_token_to_owner(&token.owner_id, &token_id); //-- passing the borrowed of token owner id and its id - adding current token to the owner; it'll insert a new token with its id and the owner id into the tokens_per_owner field

                    ...
                    Ok(otp_req) => { //-- we got the phone number of the user
                        ...
                        match serde_json::from_slice::<schemas::auth::SMSResponse>(stream.buffer()){ //-- we can also use from_reader() method which is slower than from_slice() method and deserialize the bytes of json text directly into the SMSResponse struct - the generic type of from_slice() method is SMSResponse struct - mapping (deserializing) the bytes of json text into the SMSResponse struct
                            Ok(sms_response) => {
                                ...
                                let utf_bytes_dereference_from_box = &*boxed_utf8_bytes_using_box_slcie; //-- borrow the ownership of the dereferenced boxed_utf8_bytes_using_box_slcie using & to convert it to &[u8] with a valid lifetime since the dereferenced boxed_utf8_bytes_using_box_slcie has unknown size at compile time thus working with u8 slice needs to borrow them from the memory to have their location address due to implemented ?Sized for [u8]

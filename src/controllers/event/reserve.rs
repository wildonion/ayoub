...
// -------------------------------------------------------------------------
pub async fn mock_reservation(db: Option<&Client>, api: ctx::app::Api) -> GenericResult<hyper::Response<Body>, hyper::Error>{

    
    api.post("/event/reserve/mock", |req, res| async move{
        ...
        match middlewares::auth::pass(req).await{
            Ok((token_data, req)) => { //-- the decoded token and the request object will be returned from the function call since the Copy and Clone trait is not implemented for the hyper Request and Response object thus we can't have borrow the req object by passing it into the pass() function therefore it'll be moved and we have to return it from the pass() function   
                ...
                if middlewares::auth::user::exists(db, _id, username.clone(), access_level).await{ //-- finding the user with these info extracted from jwt - cloning username is required since we want to build the init_player_info instance later

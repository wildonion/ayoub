...
pub async fn home(api: ctx::app::Api) -> Result<hyper::Response<Body>, hyper::Error>{
    api.get("/auth/home", |req, res| async move{
        ...
        if req_header.contains_key("authorization"){
            let token = &req_header["authorization"].to_str().unwrap()[7..];
            ...
            match utils::jwt::deconstruct(token).await{
                Ok(decoded_token) => {
                    ...
                    let greeting_msg = format!("Welcome {}", username);
                    ...
                    let response_body = ctx::app::Response::<String>{ //-- we have to specify a generic type for data field in Response struct which in our case is String type
                        data: Some(greeting_msg), //-- deserialize_from_json_into_struct is of type UserInfo struct 
    ...
    api.post("/auth/check-token", |req, res| async move{
        ...
        match serde_json::from_reader(whole_body_bytes.reader()){
            Ok(value) => { //-- making a serde value from the buffer which is a future IO stream coming from the client
                ...
                match serde_json::from_str::<schemas::auth::CheckTokenRequest>(&json){ //-- the generic type of from_str() method is LoginRequest struct - mapping (deserializing) the json into the LoginRequest struct

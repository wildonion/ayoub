...
pub struct MarketContract{ // NOTE - UnorderedMap is an iterable implementation of the map where as the LookupMap is a none iterable of the map
    ...
    pub by_owner_id: LookupMap<AccountId, UnorderedSet<ContractAndTokenId>>, //-- account_id: sale_id - keeping the track of all the sale ids which is made up of the `nft_contract_actor_account_id + DELIMETER + token_id` inside a set for every account_id

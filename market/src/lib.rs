...
// NOTE - our Market contract has all the minted nfts info inside of it with a mapping between their metadata and the owner
...
// NOTE - Market contract struct contains some data structures to store on chain infos about tokens and their owners at runtime
...
pub struct Market{ //-- can't implement Default trait for this contract cause Default is not implemented for LazyOption, LookupMap and UnorderedMap structs - our contract keeps track of some mapping between owner_id, token_id and the token_metadata inside some collections
    pub owner_id: AccountId, //-- contract owner who called the initialization method, can be anyone
...
#[near_bindgen] //-- implementing the #[near_bindgen] proc macro attribute on Market struct to compile all its methods to wasm so we can call them in near cli
impl Market{ //-- we'll add bytes to the contract by creating entries in the data structures - we've defined the init methods of the Market contract struct in here cause the lib.rs is our main crate
   ...
   NFTContractMetadata{
                ...
                name: "BlueRangene Family Tree NFT Marketplace Contract".to_string(),

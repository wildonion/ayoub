...
pub fn refund_deposit(storage_used: u64){ //-- refunding the initial deposit based on the amount of storage that was used up - all balances are of type u128 to cover the yocto$NEAR amounts
...
/*
    ---------------------------------------------------------------------------------------------------------------------------------------------------------------------
        ...
        | Data Collision Explanation 
         ===========================
        | we need a unique key for each object from near collections, if two near collections share the same key, they share the 
        ...
        | to avoid data collision, since the hash of the address of the current variant in enum always is unique thus we can have two keys with 
        | the same name but different values and different hashes. 
        |
        |
        |
        |
...
pub enum Storagekey{ //-- defining an enum based unique storage key for every near collection used on chain 
    TokensPerOwner, 

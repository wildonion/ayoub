...
// NOTE - inside all smart contracts are methods which are transactions and in order to call them from other contracts we have to use cross contract call promise feature by defining a trait and bound it to #[ext_contract()] proc macro attribute inside the caller contract by doing this we're extending the receiver_id's contract actor (a hypothetical contract of course) interface which contains a method that must be scheduled to call inside the receiver_id's contract actor and get the result of it inside the callback method of the caller contract
...
impl NoneFungibleTokenCore for Market{ //-- implementing the NoneFungibleTokenCore trait for our main Market contract struct to extend its interface; bounding the mentioned trait to the Market contract struct to query NFT core (nft_* methods standards) infos
    ...
    fn nft_approve(&mut self, token_id: TokenId, account_id: AccountId, msg: Option<String>){ //-- allow a specific account_id like a marketplace contract to approve a token_id on behalf of the owner and list it on the marketplace

        utils::panic_one_yocto(); //-- ensuring that the user has attached exactly one yocot$NEAR to the call to pay for the storage and security reasons (only those caller that have at least 1 yocot$NEAR can call this method; by doing this we're avoiding DDOS attack on the contract) on the contract by forcing the user to sign the transaction with his/her full access key; we'll refund any excess storage later after calculating the required storage cost for adding new approved account_id 
        ...
        if &env::predecessor_account_id() != &token.owner_id{ //-- since we can borrow the token.owner_id String thus both side of the condition must be equal together means we have to borrow the env::predecessor_account_id() String too - the last (current) caller of this method must be the owner of the token cause only the token owner must call this method to approve an account to transfer the token on his/her behalf
            env::panic_str("The Caller Is Not The Owner of The Token!"); //-- &str allocates low cost storage than the String which will get usize (usize is 64 bits or 24 bytes on 64 bits arch) * 3 (pointer, len, capacity) bytes cause it's just the size of the str itself on either stack, heap or binary which is equals to its length of utf8 bytes and due to its unknown size at compile time we must borrow it by taking a pointer to its location
        }
        ...
        let storage_used = if is_new_approval{
            ...
            0 //-- 0 storage is used if it wasn't a new approval and only attaching 1 yocot$NEAR would be enough
        };
        ...
        refund_deposit(storage_used); //-- depositing some $NEARs based on used bytes in the contract and get pay back the remaining deposit or any excess that is unused at the end by refunding the caller account; if the caller didn't attach enough it'll panic

        
        if let Some(msg) = msg{ //-- if msg wan't None then we initiate and schedule a cross contract call and call promise method the nft_on_approve() method to be executed it on the account that we're giving access to which means someone with an NFT can list it on an approved account like the marketplace account by passing in a message that could be properly decoded to allow users purchases his/her NFT on the marketplace
        ext_non_fungible_approval_receiver::ext(account_id) //-- the account_id that this method must be called and executed inside which is the one who is approved for transferring the token on behalf of the owner and is the one who has access to do this - account_id param is the one who is responsible for making this call cause nft_on_approve() is a private method means the caller must be the signer or the one who initiated, owned and signed the contract or the account of the contract itself or the sender him/her-self to mutate the state of the contract on chain thus we have to pass the approved contract actor account which is the owner of receiver_id's contract actor - no need to clone the current_account_id cause we're passing it by reference or as a borrowed type
            .with_attached_deposit(NO_DEPOSIT) //-- no deposit is required from the caller for calling the nft_on_approve() cross contract call promise method cause it's a private method inside the receiver_id's contract actor
            .with_static_gas(env::prepaid_gas() - GAS_FOR_NFT_APPROVE) //-- prepaid_gas() method returns the amount of gas attached to the call via near cli that can be used to pay the gas fees | attached gas - required gas for calling nft_approve() method is the total gas fee which will be deposited in yocot$NEAR from the caller wallet for this transaction call
            .nft_on_approve( //-- initiating the receiver's corss contract call by creating a transaction which is a promise (future object) ActionReceipt object which returns obviously a promise or a future object which contains an async message including the data coming from the receiver_id's contract actor once it gets executed - calling the nft_on_approve() cross contract call promise method on the receiver side from the extended receiver_id's contract actor interface which is `ext_non_fungible_approval_receiver`; by calling this method we can do whatever we want inside the receiver_id's contract actor which is the account that we're giving access and approving to transfer the token on behalf of the owner - alling the nft_on_approve() cross contract call promise method to schedule an ActionReceipt object which contains async message or a future object which must be solved by executing the mentioned method the inside token.owner_id's contract actor
                token_id,
                token.owner_id,
                approval_id, //-- this is the approval_id of the account_id who has access to transfer the token on behalf of the owner
                msg,
            )
            .as_return(); //-- returning this promise without getting its result using a callback
        }
...
trait NoneFungibleTokenApprovalsReceiver{ //-- this trait which contains the cross conract call methods will extend the interface of the receiver_id's contract actor with a name inside the #[ext_contract()] proc macro which specifies the extended interface contract name  

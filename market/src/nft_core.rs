...
// NOTE - we don't need to calculate the total storages used inside the nft_transfer() and nft_transfer_call() methods even though we've forced the caller to attached 1 yocot$NEAR cause in those methods we first remove the token from self.tokens_per_owner field then add the token to self.tokens_per_owner with a new owner_id therefore the amount of used storage would be 0 in total since the remvoing process will make some space for adding process which the total usded bytes will be the same as before 
...
// NOTE - based on the above rule since nft_resolve_transfer() method is a private method (only the owner of the contract can call it) in order to call it from other method inside this contract (can't call it from the cli unless the caller must be the contract owner) we have to attach the #[near_bindgen] proc macro attribute on top of the impl NoneFungibleTokenResolver for <Contract> 
...
// NOTE - if a function requires a deposit, we need a full access key of the user to sign that transaction
...
impl NoneFungibleTokenCore for Market{ //-- implementing the NoneFungibleTokenMetadata trait for our main Market contract struct (or any contract); bounding the mentioned trait to the Market contract struct to query nft metadata infos
    ...
    fn nft_transfer_call(&mut self, receiver_id: AccountId, token_id: TokenId, approval_id: Option<u64>, memo: Option<String>, msg: String) -> PromiseOrValue<bool>{ //-- returning a promise or a value which can be either true if the token was transferred from the sender_id's contract actor to the receiver_id's contract actor or false otherwise - we've defined the self to be mutable and borrowed cause we want to mutate some fields and have the isntance with a valid lifetime after calling this method on it
        ...
        */
        ...
        extend_receiver_contract_for_none_fungible_token::ext(receiver_id.clone()) //--  we're cloning the receiver_id to avoid moving cause we want to use it inside the nft_resolve_transfer() method - the account_id that this method must be called and executed inside since the account_id param is the one who is responsible for making this call cause nft_on_transfer() cross contract call promise method is a private method inside the receiver_id's contract actor means the caller must be the signer or the one who initiated, owned and signed the contract or the account of the contract itself to mutate the state of the contract on chain thus we have to pass the receiver account_id cause it's the owner of receiver_id's contract actor - no need to clone the receiver_id cause we're passing it by reference or as a borrowed type
            .with_attached_deposit(NO_DEPOSIT) //-- no deposit is required from the caller for calling nft_on_transfer() cross contract call promise method cause it's a private method inside the receiver_id's contract actor 
            ...
            .nft_on_transfer( //-- initiating the receiver's corss contract call by creating a transaction which is a promise (future object) ActionReceipt object which returns obviously a promise or a future object which contains an async message including the data coming from the receiver_id's contract actor once it gets executed - calling the nft_on_transfer() cross contract call promise method on the receiver side from the extended receiver_id's contract actor interface which is `extend_receiver_contract_for_none_fungible_token`
                ...
                ////////////
                Self::ext(env::current_account_id()) //-- the account_id that this method must be called and executed inside which is the current_account_id() and is the one who owns this contract - account_id param is the one who is responsible for making this call cause nft_resolve_transfer() is a private method means the caller must be the signer or the one who initiated, owned and signed the contract or the account of the contract itself or the sender him/her-self to mutate the state of the contract on chain thus we have to pass the current_id's or the sender_id's contract actor which is the owner of this contract actor - no need to clone the current_account_id cause we're passing it by reference or as a borrowed type
                    .with_attached_deposit(NO_DEPOSIT) //-- no deposit is required from the caller for calling the nft_resolve_transfer() callback method cause it's a private method inside this contract actor account
                    ...
                    .nft_resolve_transfer( //-- calling nft_resolve_transfer() method from the extended interface of the current contract actor (our own contract) which is the `extend_this_contract` contract
                        transferred_token.owner_id.clone(), 
                        receiver_id, 
                        token_id, 
...
#[near_bindgen] //-- implementing the #[near_bindgen] proc macro attribute on the trait implementation for the Market contract struct in order to have a compiled wasm trait methods for this contract struct so we can call it from the near cli
impl NoneFungibleTokenResolver for Market{ //-- implementing the NonFungibleTokenResolver trait for our main Market contract struct (or any contract); extending the Market contract struct interface to define the body of cross contract call promise methods which is the callback method for the current or the sender contract actor to fill the pending promise ActionReceipt object with the promise DataReceipt object coming from the receiver_id's contract actor
        ...
        let mut token = match self.tokens_by_id.get(&token_id){ //-- getting the token object related to the token_id (passed by reference to borrow it) if there is some token object from the self.tokens_by_id LookupMap
            Some(token) => {
                if token.owner_id != receiver_id{ //-- if the current token owner_id wasn't equaled to the current receiver_id means that this token doesn't belong to this receiver anymore since it might be transferred successfully to another receiver_id's contract actor thus we must return true anyway :)
                    ...
                    env::log_str(token_status_message.as_str()); //-- &str allocates low cost storage than the String which will get usize (usize is 64 bits or 24 bytes on 64 bits arch) * 3 (pointer, len, capacity) bytes cause it's just the size of the str itself on either stack, heap or binary which is equals to its length of utf8 bytes and due to its unknown size at compile time we must borrow it by taking a pointer to its location

...
impl Market{ //-- we've defined the following methods of the Market contract struct in this crate cause this crate is related to all internal calculation functions and methods - we don't need to add #[near_bindgen] proc macro attribute on this impl cause these are none exporting methods and won't compile to wasm to call them from cli 
    ...
    pub fn internal_transfer(&mut self, sender_id: &AccountId, receiver_id: &AccountId, token_id: &TokenId, approval_id: Option<u64>, memo: Option<String>) -> Token{ //-- transferring a specific NFT to a specific receiver_id - always take a reference to String (token_id) to borrow it to prevent from moving and cloning
        ...
        if sender_id != &token.owner_id{ //-- the owner_id inside the found token object must be the one who is transferring this NFT if not we then check if the sender is inside the approved_account_ids field otherwise we panic - the caller of the transferring method must be the owner of the NFT
            if !token.approved_account_ids.contains_key(sender_id){ //-- the token approved_account_ids hashmap must contains the sender_id key otherwise we panic
                env::panic_str("Unauthorized! Token is Not for You & You Are Not An Approved Account To Transfer This NFT"); //-- &str allocates low cost storage than the String which will get usize (usize is 64 bits or 24 bytes on 64 bits arch) * 3 (pointer, len, capacity) bytes cause it's just the size of the str itself on either stack, heap or binary which is equals to its length of utf8 bytes and due to its unknown size at compile time we must borrow it by taking a pointer to its location
            }
            ...
            if let Some(enforced_approval_id) = approval_id{ //-- if there was an approval_id we must check that the sender's approval_id is the same as the once inside the approved_account_ids field
                if token.approved_account_ids.contains_key(sender_id){ //-- checking that approved_account_ids field contains the approval_id related to the sender_id
                    ...
                    if actual_approved_id != &enforced_approval_id{ //-- we must borrow the enforced_approval_id using & cause the left hand side of the condition is a borrowed type
                        env::panic_str("The Actual Approved Id Is Different From The Given Approval Id"); //-- &str allocates low cost storage than the String which will get usize (usize is 64 bits or 24 bytes on 64 bits arch) * 3 (pointer, len, capacity) bytes cause it's just the size of the str itself on either stack, heap or binary which is equals to its length of utf8 bytes and due to its unknown size at compile time we must borrow it by taking a pointer to its location
                    }
                } else{ //-- we didn't found any approval_id related to the sender_id
                    env::panic_str("This Sender Is Not An Approved Account"); //-- &str allocates low cost storage than the String which will get usize (usize is 64 bits or 24 bytes on 64 bits arch) * 3 (pointer, len, capacity) bytes cause it's just the size of the str itself on either stack, heap or binary which is equals to its length of utf8 bytes and due to its unknown size at compile time we must borrow it by taking a pointer to its location
                }
            }
        }
        ...
        if &token.owner_id == receiver_id{ //-- we must borrow the token.owner_id using & cause the right hand side of the condition is a borrowed type  
            env::panic_str("The Toke Owner and the Receiver Should'nt be the Same :/"); //-- at this state on the chain we must make sure that the sender_id is not sending the token to themselves - &str allocates low cost storage than the String which will get usize (usize is 64 bits or 24 bytes on 64 bits arch) * 3 (pointer, len, capacity) bytes cause it's just the size of the str itself on either stack, heap or binary which is equals to its length of utf8 bytes and due to its unknown size at compile time we must borrow it by taking a pointer to its location
        }
    ...
    pub fn internal_remove_token_from_owner(&mut self, account_id: &AccountId, token_id: &TokenId){ //-- we've defined the self to be mutable and borrowed cause we want to remove a record from the self.tokens_per_owner LookupMap - removing the token from the owner's set of token_id(s)
        ...
        let mut tokens_set = if let Some(tokens_set) = self.tokens_per_owner.get(&account_id){ //-- getting the set of all token_id(s) of a specific owner inside the self.tokens_per_owner LookupMap
            ...
            env::panic_str("Token Should be Owned by the Sender :/"); //-- &str allocates low cost storage than the String which will get usize (usize is 64 bits or 24 bytes on 64 bits arch) * 3 (pointer, len, capacity) bytes cause it's just the size of the str itself on either stack, heap or binary which is equals to its length of utf8 bytes and due to its unknown size at compile time we must borrow it by taking a pointer to its location 
        };
